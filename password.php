...
E_USER_WARNING);
        ...
        if (!is_int($algo)) {
            trigger_error("password_hash() expects parameter 2 to be 
long, " . gettype($algo) . " given", 
E_USER_WARNING);
            return null;
        }
        $resultLength = 0;
...
E_USER_WARNING);
        ...
        if (isset($options['salt'])) {
            switch (gettype($options['salt'])) {
                ...
                case 'resource':
                default:
                    trigger_error('password_hash(): Non-string salt 
parameter supplied', E_USER_WARNING);
                    return null;
            }
            if (PasswordCompat\binary\_strlen($salt) < 
$required_salt_len) {
                trigger_error(sprintf("password_hash(): Provided salt is 
too short: %d expecting %d", 
...
function_exists('openssl_random_pseudo_bytes')) {
            ...
            if (!$buffer_valid || PasswordCompat\binary\_strlen($buffer) 
< $raw_salt_len) {
...
approach
    ...
    function password_verify($password, $hash) {
        if (!function_exists('crypt')) {
            trigger_error("Crypt must be loaded for password_verify to 
function", E_USER_WARNING);
            return false;
        }
        $ret = crypt($password, $hash);
